...
### 1. Player Already Has Account with Different Email

**Scenario:** Player was added to lineup, but already has an account from another club.

**Current Design:** Not supported. Each player_id is unique to one club. If player plays for multiple clubs, they would have separate player records and accounts.

**Future Enhancement:** Could add a `user_players` linking table to allow one user account to have multiple player records.

### 2. Name Changes

**Scenario:** Player's name in lineup is "Marc Silva" but player wants to sign up as "Marcus Silva".

**Solution:** Player can edit name during signup. Both `players.player_name` and `users.full_name` are updated to the player's chosen name.

**Implications:**
- Historical match events still show original name from lineup
- Season statistics will show updated name
- Consider updating `events.player_name` to match (optional)

### 3. Duplicate Players in Lineup

**Scenario:** Same player appears in multiple matches, code generation creates duplicate player records.

**Solution:** Before creating player record, check if player exists by:
1. `statsbomb_player_id` (if available)
2. `jersey_number` + `club_id` combination

If player exists, skip creation and reuse existing player_id and invite_code.

### 4. Lost Invite Code

**Scenario:** Player loses their invite code.

**Solution:** Coach can view all unlinked players and their invite codes in the coach portal:
- GET `/api/coach/players` returns `unlinked_players` array with `invite_code` field
- Coach can resend code to player

### 5. Typo in Invite Code

**Scenario:** Player enters "MRC-1287" instead of "MRC-1827".

**Solution:**
- Backend returns 404 error
- Frontend shows "Invalid invite code" message
- Player can retry

**Enhancement:** Could implement fuzzy matching to suggest corrections (e.g., "Did you mean MRC-1827?")

### 6. Race Condition - Multiple Signups with Same Code

**Scenario:** Two devices try to sign up with same code simultaneously.

**Solution:**
- Database transaction wraps entire signup process
- Check `is_linked = FALSE` inside transaction
- If already linked, transaction fails with 409 error
- Second device receives error and cannot complete signup

### 7. Profile Photo Upload

**Scenario:** Player uploads profile photo during signup.

**Recommended Flow:**
1. Frontend uploads image to cloud storage first (separate API or direct upload)
2. Cloud storage returns URL
3. Frontend includes URL in signup request
4. Backend stores URL in `profile_image_url`

**Alternative Flow:**
1. Frontend sends signup request without photo
2. After signup complete, frontend uploads photo
3. Frontend calls PUT `/api/player/profile` to update photo

### 8. Incomplete Signup

**Scenario:** Player completes Step 1 but closes app before completing Step 2.

**Solution:**
- No data is saved after Step 1 (verification is read-only)
- Player can restart process from Step 1
- Invite code remains valid (`is_linked` still FALSE)

### 9. Player Wants to Change Email After Signup

**Scenario:** Player signed up with wrong email.

**Solution:** Not covered in current design. Would need:
- Email change endpoint
- Email verification flow
- Security considerations (prevent account takeover)
